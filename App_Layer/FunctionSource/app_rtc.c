...
void App_RtcMin_Process(void)
{
    ...
    #if(AIR_PRESSURE_SUPPORT == 1)
    //天气处理(带气压才开启)
    if ((appMinCnt % WEATHER_REQ_TIME == 0) && (systermConfig.weatherSwitch == SWITCH_ON))
    {
        //每分钟采样一次气温
        msg.id                     = WEATHER_ID;
    ...
    //fix : 有概率出现一直不检测电量的情况，原因:原来的code用backstageTimeCounterSec这是秒的计数，此处
    // 是以分钟作为计数，在1min一次，backstageTimeCounterSec % BAT_CHECK_TIME有可能一直都不会==0
    if ((WinDowHandle != WIN_STORE_HANDLE) && (backstageTimeCounterMin % BAT_CHECK_TIME == 0))    
    {
        msg.id                     = BAT_ID;
    ...
    //变量越界，复位
    if(backstageTimeCounterMin == 0xFFFFFFFF)
        backstageTimeCounterMin = 0x00;
    if(appMinCnt == 0xFFFFFFFF)
        appMinCnt = 0x00;
	#endif
}
