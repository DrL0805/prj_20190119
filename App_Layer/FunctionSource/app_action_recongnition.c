...
void setActionThreshold(unsigned short up1, unsigned short up2, unsigned short down1, unsigned short down2)
{
    threshold[0] = up1;
    threshold[1] = up2;
    threshold[2] = down1;
...
// *******************************************************************************
// *Funtion name:actionRecongnition
...
// *Output:none
// /*****************************************************************************
int actionRecongnition(unsigned char xyzDataTemp[3])
{
    unsigned char   i;
...
void App_Action_Recongnition(uint8 *xyzData)
{
    ...
    setActionThreshold(350, 300, 350, 300);

    if(phoneState.state == PHONE_STATE_AUTHOR)
    {
        temp2 = actionRecongnition(xyzData);
        if(temp2 == 1)
        {
            rejectCall++;
            if(rejectCall >= 2)
     ...
     {
         setActionThreshold(600, 400, 600, 400);//450, 400, 450, 400//门限值改大，避免马达震动造成动作识别
         temp2 = actionRecongnition(xyzData);
         if(temp2 == 1)
         {
            rejectCall++;
             if(rejectCall >= 2)
             {
                 ...
                 if(UnreadRemindnID == REMIND_PHONECALL)
                 {
                    UnreadRemindnID = REMIND_NULL;
                 }
