...
#define WIN_SWITCH_PERIOD      		4 //窗口切换周期：单位：APP_1SEC_TICK / 2

#define WIN_TIME_1SEC 				3//(1 * WIN_SWITCH_PERIOD)
#define WIN_TIME_2SEC 				5//(2 * WIN_SWITCH_PERIOD)
#define WIN_TIME_3SEC 				7//(3 * WIN_SWITCH_PERIOD)
#define WIN_TIME_4SEC 				9//(4 * WIN_SWITCH_PERIOD)
#define WIN_TIME_5SEC 				11//(5 * WIN_SWITCH_PERIOD)
#define WIN_TIME_6SEC 				13//(6 * WIN_SWITCH_PERIOD)
#define WIN_TIME_7SEC 				15//(7 * WIN_SWITCH_PERIOD)
#define WIN_TIME_8SEC 				17//(8 * WIN_SWITCH_PERIOD)
#define WIN_TIME_9SEC 				19//(9 * WIN_SWITCH_PERIOD)
#define WIN_TIME_10SEC 				21//(10 * WIN_SWITCH_PERIOD)


#define ROUND_VERTICAL_PIXEL 		4
#define ROUND_HORIZONTAL_PIXEL 		4
...
typedef struct 
{
	...
	uint16 		axisX;		//菜单坐标
	uint16 		axisY;		//菜单坐标
	uint16 		(*callback)(uint16 sysHandle,menuMessage message);	//菜单回调处理函数
}WinMenu;
...
typedef enum 
{
	...
	WIN_SWING_HANDLE,
	WIN_STORE_HANDLE,
	WIN_IDLE_HANDLE,		//休眠窗口，显示关闭
	WIN_INVALID_HANDLE	 	= 0xffff,
}SysWinHandle;
