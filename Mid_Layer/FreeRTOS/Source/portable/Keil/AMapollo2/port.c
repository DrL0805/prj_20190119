...
#ifndef AM_STIMER_FREQ
#define AM_STIMER_FREQ  32768
#endif

#ifndef AM_HAL_STIMER_CFG_ENABLE
#define AM_HAL_STIMER_CFG_ENABLE (AM_HAL_STIMER_XTAL_32KHZ|AM_HAL_STIMER_CFG_COMPARE_A_ENABLE)
#endif
...
#define AM_STIMER_CLOCK         AM_HAL_STIMER_XTAL_32KHZ
#else
#error "Unsupported STimer Clock for FreeRTOS tick"
#endif
...
#define AM_CTIMER_CLOCK         AM_HAL_CTIMER_XT_32_768KHZ
#else
#error "Unsupported CTimer Clock for FreeRTOS tick"
#endif
...
void vPortSuppressTicksAndSleep( TickType_t xExpectedIdleTime )
{
...
#else
    SMDrv_CTimer_Stop(SYSTICK_CTIMER_MODULE);

    // Adjust for the time already elapsed
    elapsed_time = SMDrv_CTimer_ReadCount(SYSTICK_CTIMER_MODULE);
#endif
	...
	{
#ifndef AM_FREERTOS_USE_STIMER_FOR_TICK 
        SMDrv_CTimer_Start(SYSTICK_CTIMER_MODULE);
#endif
		/* Re-enable interrupts - see comments above __disable_irq() call
		above. */
		__enable_irq();
	...
	else
	{
...
#else
        SMDrv_CTimer_Clear(SYSTICK_CTIMER_MODULE);
        SMDrv_CTimer_SetCmpValue(SYSTICK_CTIMER_MODULE,ulReloadValue);
        SMDrv_CTimer_Start(SYSTICK_CTIMER_MODULE);
#endif
...
#else
        SMDrv_CTimer_Stop(SYSTICK_CTIMER_MODULE);
        New_Timer = SMDrv_CTimer_ReadCount(SYSTICK_CTIMER_MODULE);
        ...
        if ((New_Timer == 0) && ((am_hal_ctimer_int_status_get(false) & (1 << SMDrv_CTimer_GetTimerNumber(SYSTICK_CTIMER_MODULE)*2))))
        {
            ...
            am_hal_ctimer_int_clear((1 << SMDrv_CTimer_GetTimerNumber(SYSTICK_CTIMER_MODULE) *2));
        } else
        {
...
#else
        SMDrv_CTimer_Clear(SYSTICK_CTIMER_MODULE);
        SMDrv_CTimer_SetCmpValue(SYSTICK_CTIMER_MODULE,ulTimerCountsForOneTick);
        SMDrv_CTimer_Start(SYSTICK_CTIMER_MODULE);
#endif
...
//
//*****************************************************************************
void xPortCTimerTickHandler(void)
{
    //设置软件中断，在软件中断中进行OS Tick自增，及task switch
    //不在ctimer中断中做的原因:在系统中ctimer中断的优先级最高，频繁切换task会影响
    //表针做行针的动作
    am_hal_interrupt_pend_set(AM_HAL_INTERRUPT_SOFTWARE0);
}
...
void vPortSetupTimerInterrupt( void )
{
   /* Calculate the constants required to configure the tick interrupt. */
    ...
    //step 1: set SVC for tick
    SMDrv_SYSTick_ClockInit();
   
    //step 2: 开启ctimer作滴答时钟
    //open a ctimer for systick, ctimer interrupt prio,and start the ctimer
    SMDrv_CTimer_Open(SYSTICK_CTIMER_MODULE,ulTimerCountsForOneTick,xPortCTimerTickHandler);
...
void vPortSetupTimerInterrupt( void )
{
    ...
    {
        .ui32Link = 1,
        .ui32TimerAConfig = (AM_CTIMER_CLOCK |
