...
static void appSlaveConnUpdate(dmEvt_t *pMsg, appConnCb_t *pCb)
{
  if (pAppUpdateCfg->idlePeriod != 0)
  {
    ...
    if (pMsg->hdr.status == HCI_SUCCESS)
    {
      ...
      {
        //fix 2:测试iphone 7手机连接，参数更新状态成功，但是连接参数比设置的max还大的情况，会一直做参数更新动作
        // 此iphone 7最小值是24， 软件设置10/20就会出现一直更新参数，
        // 出现这种情况的时候，调大连接参数中connIntervalMax值，调调整为connIntervalMax += 24-20 + (10 - 6%10) = 30
        if((((hciLeConnUpdateCmplEvt_t*)pMsg)->connInterval > pAppUpdateCfg->connIntervalMax) && 
            (((hciLeConnUpdateCmplEvt_t*)pMsg)->status == 0x00))
        {
            uint16_t u16dif = ((hciLeConnUpdateCmplEvt_t*)pMsg)->connInterval - pAppUpdateCfg->connIntervalMax;
            pAppUpdateCfg->connIntervalMax += (u16dif + (10 - u16dif % 10)); //调大max值，如差值是16，max则加大20
        }
        //fix: 2018.6.5
...
static void appSecPairCmpl(dmEvt_t *pMsg, appConnCb_t *pCb)
{
  ...
  if (pCb->bondByPairing)
  {
    ...
    if (pCb->dbHdl != APP_DB_HDL_NONE)
    {
      ...
      AppSaveNewRecInNvm(pCb->connId, pCb->dbHdl);
    }
  }
}
...
/*************************************************************************************************/
/*!
 *  \fn     appPrivRemDevFromResListInd
 *
