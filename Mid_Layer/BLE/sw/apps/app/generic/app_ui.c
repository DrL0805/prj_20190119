...
//for callback passkey to mcu
static ble_echo  pDisplayPasskey_CB = NULL;
#endif
...
void AppUiAction(uint8_t event)
{
  switch (event)
  {
...
#if(BLE_PASSKEY_PAIRNG_ENABLE == 1)
        if(pDisplayPasskey_CB != NULL)
        {
            (pDisplayPasskey_CB)(BLE_PAIRING_AUTH_STATUS,0x00,0);
        }
#endif
        break;
    case APP_UI_SEC_PAIR_FAIL:
        ...
        if(pDisplayPasskey_CB != NULL)
        {
            (pDisplayPasskey_CB)(BLE_PAIRING_AUTH_STATUS,0x01,0);
        }
#endif
        break;
...
#if(BLE_PASSKEY_PAIRNG_ENABLE == 1)
//set callback for display passkey
void AppUiDisplayCallBack(ble_echo pfun)
{
    if(pfun == NULL)
        return ;
    pDisplayPasskey_CB = pfun;
}
#endif
...
#if(BLE_PASSKEY_PAIRNG_ENABLE == 1)
    //SEGGER_RTT_printf(0,">>> Passkey: %d <<<", passkey);
    if(pDisplayPasskey_CB != NULL)
    {
        (pDisplayPasskey_CB)(BLE_PAIRING_PASSKEY_ECHO,passkey,0);
    }
#endif
}
