...
//
//*****************************************************************************
sm_bootloader_image_t *g_psBootImage = (sm_bootloader_image_t *) FLAG_PAGE_LOCATION;
...
static uint8 SMOTA_Flash_CheckAddrRange(uint32 address, uint32 size)
{
    static uint32 g_intFlashSize = 0;

    uint32 ui32Start = address;
    uint32 ui32End = address + size - 1;

    if (g_intFlashSize == 0) // First call
    {
        // Get chip specific info
        g_intFlashSize = SMDrv_Flash_GetTotalSize();
    }
    ...
    {
        Err_Info((0,"Error:CheckFlashAddrRange: ui32Start=0x%x\n", ui32Start));
        return FALSE;
    }
    ...
    {
        Err_Info((0,"Error:CheckFlashAddrRange: ui32End=0x%x,g_intFlashSize=0x%x\n", ui32End, g_intFlashSize));
        return FALSE;
    }
    if (USE_FLAG_PAGE )
    {
        ...
        if ( (FLAG_PAGE_LOCATION == ui32Start) ||
             ((FLAG_PAGE_LOCATION < ui32Start) && ((FLAG_PAGE_LOCATION + SMDrv_Flash_GetPageSize()) > ui32Start)) ||
             ((FLAG_PAGE_LOCATION > ui32Start) && (FLAG_PAGE_LOCATION <= ui32End)))
        {
            Err_Info((0,"Error:CheckFlashAddrRange: ui32Start=0x%x,ui32End=0x%x\n",ui32Start,ui32End));
            return FALSE;
        }
    }
    return TRUE;
...
//
//*****************************************************************************
void SMOTA_Partial_CRC32(const void *pvData, uint32 ui32NumBytes,uint32 *pui32CRC)
{
    uint32 ui32CRCIndex, i;
    uint8 *pui8Data;
    uint32 ui32TempCRC = *pui32CRC;

    pui8Data = (uint8 *) pvData;
...
//
//*****************************************************************************
uint8 SMOTA_Validate_Structure(uint32 *pInfo, uint32 size)
{
    uint32 ui32Crc = 0;
    // CRC value is the last 4 bytes of the structure
    uint32 *pCrc = pInfo + size / 4 - 1;
    // Compute and validate CRC of the structure
    SMOTA_Partial_CRC32(pInfo, size - 4, &ui32Crc);
    if (*pCrc != ui32Crc)
    {
        Err_Info((0,"Error:CRC fail! *pCrc =%d,ui32Crc=%d\n",*pCrc,ui32Crc));
        return FALSE;
    }
...
//
//*****************************************************************************
uint8 SMOTA_GetMainImageInfo(uint32 *pui32LinkAddr, uint32 *pui32Length)
{
    uint8 bValid = FALSE;
    //
    ...
    if ( USE_FLAG_PAGE )
    {
        ...
        if ( SMOTA_Validate_Structure((uint32 *)g_psBootImage, sizeof(*g_psBootImage)) )
        {
            *pui32LinkAddr = (uint32)g_psBootImage->pui32LinkAddress;
...
// 返回参数: 
//**********************************************************************
int32 SMOTA_Flash_ReadPage(uint32 ui32DestAddr, uint32 *pSrc, uint32 ui32Length)
{
    if (SMOTA_Flash_CheckAddrRange((uint32)pSrc, ui32Length))
    {
        memcpy((uint8 *)ui32DestAddr, (uint8 *)pSrc, ui32Length);
        return 0;
    }
    return -1;
}
...
//**********************************************************************
int32 SMOTA_Flash_WritePage(uint32 ui32DestAddr, uint32 *pSrc, uint32 ui32Length)
{
    if (SMOTA_Flash_CheckAddrRange(ui32DestAddr, ui32Length))
    {
        SMDrv_Flash_ProgramPage(ui32DestAddr, pSrc, ui32Length);
        return 0;
    }
...
// 说    明:该接口将擦除整个page内容
//**********************************************************************
int32 SMOTA_Flash_ErasePage(uint32 ui32DestAddr)
{
    if (SMOTA_Flash_CheckAddrRange(ui32DestAddr, 4))
    {
        SMDrv_Flash_ErasePage(ui32DestAddr);
        return 0;
    }
