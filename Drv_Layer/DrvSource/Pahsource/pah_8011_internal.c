...
pah_ret pah_8011_verify_product_id(void)
{
    ...
    if (data != PAH_PRODUCT_ID)  //0x11
    {
        ////RTT_PRINTF(0,"pah_8011_verify_product_id() fail. id = 0x%2X \n", data);
        ret = pah_err_verify_device_fail;
        goto FAIL;
    }
    
    //RTT_PRINTF(0,"pah_8011_verify_product_id() success. id = 0x%2X \n", data);

    return pah_success;//说明校验通过

FAIL:
    //RTT_PRINTF(0,"pah8011_verify_product_id() failed. ret = %d \n", ret);
    return ret;
}
...
pah_ret pah_8011_write_touch_package_setting(void)
{
    ...
    uint8 Cmd_TouchDetection_Th_Hi_15_8;  // Touch upper boundary
    uint8 Cmd_TouchDetection_Th_Lo_15_8;  // Touch lower boundary
    uint8 Cmd_adc_osr_t;  // adc oversampling rate
    uint8 T_ledbias_sel;
...
pah_ret pah_8011_write_reg_array(const uint8 reg_array[][2], uint32 length)
{
    uint32 i = 0;
    ...
    for (i = 0; i < length; ++i)
    {
        ...
        {
            //RTT_PRINTF(0,"pah_8011_write_reg_array(). pah_comm_write fail, i = %d \n", i);
...
FAIL:
    //RTT_PRINTF(0,"pah_8011_write_init_setting() failed. ret = %d \n", ret);
    return ret;
}
